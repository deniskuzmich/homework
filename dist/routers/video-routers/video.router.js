...
exports.videoRouter.get('/:id', (req, res) => {
    const video = in_memory_db_1.db.videos.find((video) => video.id === +req.params.id);
    if (!video) {
...
exports.videoRouter.delete('/:id', (req, res) => {
    for (let i = 0; i < in_memory_db_1.db.videos.length; i++) {
        if (in_memory_db_1.db.videos[i].id === +req.params.id) {
            in_memory_db_1.db.videos.splice(i, 1);
            res.sendStatus(http_statuses_1.HTTP_STATUSES.NO_CONTENT_204);
            return;
        }
    }
